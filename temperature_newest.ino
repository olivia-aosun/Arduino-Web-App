/***************************************************************************

                     Copyright 2008 Gravitech
                        All Rights Reserved

****************************************************************************/

/***************************************************************************
 File Name: I2C_7SEG_Temperature.pde

 Hardware: Arduino Diecimila with 7-SEG Shield

 Description:
   This program reads I2C data from digital thermometer and display it on 7-Segment

 Change History:
   03 February 2008, Gravitech - Created

****************************************************************************/

#include <Wire.h> 
 
#define BAUD (9600)    /* Serial baud define */
#define _7SEG (0x38)   /* I2C address for 7-Segment */
#define THERM (0x49)   /* I2C address for digital thermometer */
#define EEP (0x50)     /* I2C address for EEPROM */
#define RED (3)        /* Red color pin of RGB LED */
#define GREEN (5)      /* Green color pin of RGB LED */
#define BLUE (6)       /* Blue color pin of RGB LED */

#define COLD (23)      /* Cold temperature, drive blue LED (23c) */
#define HOT (26)       /* Hot temperature, drive red LED (27c) */
int led = 14; 
int brightness = 0; 
int fadeAmount = 5; 

const byte NumberLookup[16] =   {0x3F,0x06,0x5B,0x4F,0x66,
                                 0x6D,0x7D,0x07,0x7F,0x6F, 
                                 0x77,0x7C,0x39,0x5E,0x79,0x71};


/* Function prototypes */
void Cal_temp (int&, byte&, byte&, bool&);
void Dis_7SEG (int, byte, byte, bool);
void Send7SEG (byte, byte);
void SerialMonitorPrint (byte, int, bool);
void UpdateRGB (byte);

/***************************************************************************
 Function Name: setup

 Purpose: 
   Initialize hardwares.
****************************************************************************/

void setup() 
{ 
  Serial.begin(BAUD);
  Wire.begin();        /* Join I2C bus */
  pinMode(led, OUTPUT); 
  pinMode(RED, OUTPUT);    
  pinMode(GREEN, OUTPUT);  
  pinMode(BLUE, OUTPUT);   
  delay(500);          /* Allow system to stabilize */
} 

/***************************************************************************
 Function Name: loop

 Purpose: 
   Run-time forever loop.
****************************************************************************/
 
void loop() 
{ 
  int Decimal;
  byte Temperature_H, Temperature_L, counter, counter2;
  bool IsPositive;
  bool displayOn = true;;
  String input;
  
  /* Configure 7-Segment to 12mA segment output current, Dynamic mode, 
     and Digits 1, 2, 3 AND 4 are NOT blanked */
     
  Wire.beginTransmission(_7SEG);   
  byte val = 0; 
  Wire.write(val);
  val = B01000111;
  Wire.write(val);
  Wire.endTransmission();
  
  /* Setup configuration register 12-bit */
     
  Wire.beginTransmission(THERM);  
  val = 1;  
  Wire.write(val);
  val = B01100000;
  Wire.write(val);
  Wire.endTransmission();
  
  /* Setup Digital THERMometer pointer register to 0 */
     
  Wire.beginTransmission(THERM); 
  val = 0;  
  Wire.write(val);
  Wire.endTransmission();
  
  /* Test 7-Segment */
  for (counter=0; counter<8; counter++)
  {
    Wire.beginTransmission(_7SEG);
    Wire.write(1);
    for (counter2=0; counter2<4; counter2++)
    {
      Wire.write(1<<counter);
    }
    Wire.endTransmission();
    delay (250);
  }
  
  while (1)
  {
    Wire.requestFrom(THERM, 2);
    Temperature_H = Wire.read();
    Temperature_L = Wire.read();
    input = Serial.readString(); 
    
    /* Calculate temperature */
    // by default, this is in celsius 
    Cal_temp (Decimal, Temperature_H, Temperature_L, IsPositive);
    
    /* Display temperature on the serial monitor. 
       Comment out this line if you don't use serial monitor.*/
    SerialMonitorPrint (Temperature_H, Decimal, IsPositive);
    
    /* Display temperature on the 7-Segment */
    while (input != NULL) { 
      
      // this part handles the standby feature 
      // if server sends over "f", this means that they want it in standby 
      if (input == "f") { 
          Send7SEG(4,0x40);   
          Send7SEG(3,0x40);  
          Send7SEG(2,0x40);   
          Send7SEG(1,0x40); 
          displayOn = false;  

      // if server sends over "o", this means we want display back on 
      } else if (input == "o") {
        Dis_7SEG (Decimal, Temperature_H, Temperature_L, IsPositive);
        displayOn = true;

      // this part handles changing the light on arduino
      // r = red, g = green, b = blue 
      } else if (input == "r" || input == "g" || input =="b") { 
          changeRGB(input); 

      // display temp in celsius 
      } else if (input == "c" && displayOn == true) {
        Dis_7SEG (Decimal, Temperature_H, Temperature_L, IsPositive);
      } else { 
          // else all other inputs are numbers

          // if string input has a period in it, then arduino should display fahrenheit temperature 
          int val = input.indexOf("."); 
          if (val != -1) { 
            ShowTempInFahr(input);
          } else if (isValidNumber(input) == true) { 
            // if string input doesn't have a period in it, then arduino should display sum of two numbers 
            displaySum(input);
          }
      }
      input = Serial.readString(); 
    }
    delay (1000);        /* Take temperature read every 1 second */
  }


}

/***************************************************************************
 Function Name: Cal_temp

 Purpose: 
   Calculate temperature from raw data.
****************************************************************************/
void Cal_temp (int& Decimal, byte& High, byte& Low, bool& sign)
{
  if ((High&B10000000)==0x80)    /* Check for negative temperature. */
    sign = 0;
  else
    sign = 1;
    
  High = High & B01111111;      /* Remove sign bit */
  Low = Low & B11110000;        /* Remove last 4 bits */
  Low = Low >> 4; 
  Decimal = Low;
  Decimal = Decimal * 625;      /* Each bit = 0.0625 degree C */
  
  if (sign == 0)                /* if temperature is negative */
  {
    High = High ^ B01111111;    /* Complement all of the bits, except the MSB */
    Decimal = Decimal ^ 0xFF;   /* Complement all of the bits */
  }  
}

/***************************************************************************
 Function Name: Dis_7SEG

 Purpose: 
   Display number on the 7-segment display.
****************************************************************************/
void Dis_7SEG (int Decimal, byte High, byte Low, bool sign)
{
  byte Digit = 4;                 /* Number of 7-Segment digit */
  byte Number;                    /* Temporary variable hold the number to display */
  
  if (sign == 0)                  /* When the temperature is negative */
  {
    Send7SEG(Digit,0x40);         /* Display "-" sign */
    Digit--;                      /* Decrement number of digit */
  }
  
  if (High > 99)                  /* When the temperature is three digits long */
  {
    Number = High / 100;          /* Get the hundredth digit */
    Send7SEG (Digit,NumberLookup[Number]);     /* Display on the 7-Segment */
    High = High % 100;            /* Remove the hundredth digit from the TempHi */
    Digit--;                      /* Subtract 1 digit */    
  }
  
  if (High > 9)
  {
    Number = High / 10;           /* Get the tenth digit */
    Send7SEG (Digit,NumberLookup[Number]);     /* Display on the 7-Segment */
    High = High % 10;            /* Remove the tenth digit from the TempHi */
    Digit--;                      /* Subtract 1 digit */
  }
  
  Number = High;                  /* Display the last digit */
  Number = NumberLookup [Number]; 
  if (Digit > 1)                  /* Display "." if it is not the last digit on 7-SEG */
  {
    Number = Number | B10000000;
//    Serial.print("TEMPERATURE: " + Number); 
  }
  Send7SEG (Digit,Number);  
  Digit--;                        /* Subtract 1 digit */
  
  if (Digit > 0)                  /* Display decimal point if there is more space on 7-SEG */
  {
    Number = Decimal / 1000;
    Send7SEG (Digit,NumberLookup[Number]);
    Digit--;
  }

  if (Digit > 0)                 /* Display "c" if there is more space on 7-SEG */
  {
    Send7SEG (Digit,0x58);
    Digit--;
  }
  
  if (Digit > 0)                 /* Clear the rest of the digit */
  {
    Send7SEG (Digit,0x00);    
  }  
}

/***************************************************************************
 Function Name: Send7SEG

 Purpose: 
   Send I2C commands to drive 7-segment display.
****************************************************************************/

void Send7SEG (byte Digit, byte Number)
{
  Wire.beginTransmission(_7SEG);
  Wire.write(Digit);
  Wire.write(Number);
  Wire.endTransmission();
}

/***************************************************************************
 Function Name: UpdateRGB

 Purpose: 
   Update RGB LED according to define HOT and COLD temperature. 
****************************************************************************/

void UpdateRGB (byte Temperature_H)
{
  digitalWrite(RED, LOW);
  digitalWrite(GREEN, LOW);
  digitalWrite(BLUE, LOW);        /* Turn off all LEDs. */
  
  if (Temperature_H <= COLD)
  {
    digitalWrite(BLUE, HIGH);
  }
  else if (Temperature_H >= HOT)
  {
    digitalWrite(RED, HIGH);
  }
  else 
  {
    digitalWrite(GREEN, HIGH);
  }
}

/***************************************************************************
 Function Name: SerialMonitorPrint

 Purpose: 
   Print current read temperature to the serial monitor.
****************************************************************************/
void SerialMonitorPrint (byte Temperature_H, int Decimal, bool IsPositive)
{
    Serial.print("The temperature is ");
    if (!IsPositive)
    {
      Serial.print("-");
    }
    Serial.print(Temperature_H, DEC);
    Serial.print(".");
    Serial.print(Decimal, DEC);
    Serial.print(" degrees C");
    Serial.print("\n\n");
}

/***************************************************************************
 Function Name: changeRGB

 Purpose: 
   Updates RGB LED light according to user input. 
****************************************************************************/

void changeRGB(String input) { 
  analogWrite(RED, LOW); 
  analogWrite(BLUE, LOW); 
  analogWrite(GREEN, LOW); 

    if (input == "b")
  {
    digitalWrite(BLUE, HIGH);
  }
  else if (input == "r")
  {
    digitalWrite(RED, HIGH);
  }
  else if (input == "g")
  {
    digitalWrite(GREEN, HIGH);
  }
}



/***************************************************************************
 Function Name: isValidNumber

 Purpose: 
   Determines whether the incoming string represents a number without nothing else in it 
   This is used to display the sum of two numbers on the arduino 
****************************************************************************/
boolean isValidNumber(String str){
   for(byte i=0;i<str.length();i++)
   {
      // return true only if every character in string is a digit, else return false 
      if(isDigit(str.charAt(i))) return true;
        }
   return false;
} 

/***************************************************************************
 Function Name: DisplaySumOnArduino

 Purpose: 
   This function displays the sum of two numbers on the arduino 
****************************************************************************/

void displaySum (String sumToBeShown)
{ 

  Serial.print(sumToBeShown);
  byte Digit = 4; 
  int number = sumToBeShown.toInt(); 
  int temp;

// below if/else condition is to determine which digits to set to blank based on how long the input string is 
  if (sumToBeShown.length() >4) {
    Send7SEG(4, 0x79);
    Send7SEG(3, 0x77);
    Send7SEG(2, 0x77);
    Send7SEG(1, 0xBF);
    return;
  } else if (sumToBeShown.length() == 3) {
    Digit = 3; 
    Send7SEG(4, 0x00);
  } else if (sumToBeShown.length() ==2) {
    Digit = 2; 
    Send7SEG(4, 0x00);
    Send7SEG(3, 0x00);
  } else if (sumToBeShown.length() ==1) { 
     Digit = 1; 
     Send7SEG(4, 0x00);
     Send7SEG(3, 0x00);
     Send7SEG(2, 0x00);
  }
  
  if (number > 999)                  /* When the temperature is three digits long */
  {
    temp = number / 1000;          /* Get the hundredth digit */
    Send7SEG (Digit,NumberLookup[temp]);     /* Display on the 7-Segment */
    number = number % 1000;            
    Digit--;                      /* Subtract 1 digit */    
  }
  
  if (number > 99)
  {
    temp = number / 100;           /* Get the tenth digit */
    Send7SEG (Digit,NumberLookup[temp]);     /* Display on the 7-Segment */
    number = number % 100;             
    Digit--;                      /* Subtract 1 digit */
  }

  if (number > 9) 
  { 
    temp = number/10; 
    Send7SEG (Digit,NumberLookup[temp]);  
    number = number % 10; 
    Digit--; 

  }
  
  Send7SEG (Digit,NumberLookup[number]); 
} 



/***************************************************************************
 Function Name: ShowTempInFahr

 Purpose: 
   This function is meant to display the temperature in F on the arduino 
****************************************************************************/

void ShowTempInFahr (String tempToBeConverted)
{ 
  byte Digit = 4; 

  // find where the period is so we know which digit will have it on the display 
  int wherePeriodIs = tempToBeConverted.indexOf("."); 
  String copy = tempToBeConverted; 

  // take out the period in a copy string so that we can display the number like a normal string 
  copy.replace(".", "");
  int tempWithoutPeriods = copy.toInt(); 
  int temp;

  // based on the length of the string, determine which digit we start from on the arduino
  if (copy.length() == 3) {
    Digit = 3; 
    Send7SEG(4, 0x00);
  } else if (copy.length() ==2) {
    Digit = 2; 
    Send7SEG(4, 0x00);
    Send7SEG(3, 0x00);
  } else if (copy.length() ==1) { 
     Digit = 1; 
     Send7SEG(4, 0x00);
     Send7SEG(3, 0x00);
     Send7SEG(2, 0x00);
  }

  
  if (tempWithoutPeriods > 999)                  /* When the temperature is three digits long */
  {

    temp = tempWithoutPeriods / 1000;          /* Get the hundredth digit */
    Send7SEG (Digit,NumberLookup[temp]);     /* Display on the 7-Segment */
    tempWithoutPeriods = tempWithoutPeriods % 1000;            
    Digit--;                      /* Subtract 1 digit */    
  }
  
  if (tempWithoutPeriods > 99)
  {
    temp = tempWithoutPeriods / 100;           /* Get the tenth digit */
    if(wherePeriodIs == 2 && copy.length() == 4) {  /* if the period is originally at index 2 in string, display it at this digit*/
      temp = NumberLookup[temp]; 
      temp = temp | B10000000;
      Send7SEG (Digit,temp);     
    } else { 
       Send7SEG (Digit,NumberLookup[temp]);     /* Display on the 7-Segment */
    }
    tempWithoutPeriods = tempWithoutPeriods % 100;             
    Digit--;                      /* Subtract 1 digit */
  }

  if (tempWithoutPeriods > 9) { 
    temp = tempWithoutPeriods/10; 

    if (wherePeriodIs == 2 && copy.length() == 3) {  /* if the period is originally at index 2 in string, display it at this digit*/
        temp = NumberLookup[temp]; 
        temp = temp | B10000000;
        Send7SEG (Digit,temp);  
    } else if (wherePeriodIs == 3 && copy.length() ==4) {  /* if the period is originally at index 3 in string, display it at this digit*/
      temp = NumberLookup[temp]; 
      temp = temp | B10000000;
      Send7SEG (Digit,temp);  

    } else { 
        Send7SEG (Digit,NumberLookup[temp]);     /* Display on the 7-Segment */
    }
    tempWithoutPeriods = tempWithoutPeriods % 10; 
    Digit--; 
  }
  
  Send7SEG (Digit,NumberLookup[tempWithoutPeriods]); 
  // last digit is always f for fahrenheit
  Send7SEG(1, 0x71);  /*get rid of last digit on the arduino so we only display 3 digit fahr temperature*/
} 




